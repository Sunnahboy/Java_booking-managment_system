...
import main.java.com.hallbooking.part2_admin.AdminManager;
...
public class AdminDashboard extends JFrame {
    ...
    private void initComponents() {
        setTitle("Super Admin Management System");
        ...
        JLabel titleLabel = new JLabel("Super Admin Management System");
    ...
    private JPanel createAddUserPanel() {
        ...
        String[] userTypes = {"Manager", "Customer", "Scheduler", "Admin"};
    ...
    private void toggleUserStatus(boolean block) {
        ...
        try {
            if (block) {
                adminManager.blockUser(userId);  // Call to block user
                JOptionPane.showMessageDialog(this, "User blocked successfully!");
            } else {
                adminManager.unblockUser(userId);  // Call to unblock user
    ...
    private JPanel createUpdateUserPanel() {
        ...
        JButton adminButton = createStyledButton("Admins", new Color(52, 152, 219));
    ...
    private void addUser() {
        try {
            ...
            if (!(id.startsWith("M") || id.startsWith("C") || id.startsWith("S") || id.startsWith("A"))) {
                JOptionPane.showMessageDialog(this, "User ID must start with 'M', 'C', or 'S' or 'A' ", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            adminManager.createUser(newUser);
    ...
    private void filterUsers(String criteria, String value) {
        try {
            ...
            switch (selectedUserType) {
                case "Customer":
                    filteredUsers = adminManager.filterUsers("customers.txt", criteria, value);
                    break;
                case "Scheduler":
                    filteredUsers = adminManager.filterUsers("schedulers.txt", criteria, value);
                    break;
                case "Manager":
                    filteredUsers = adminManager.filterUsers("users.txt", criteria, value).stream()
                            .filter(user -> user.getId().startsWith("M") && !user.getUserType().equalsIgnoreCase("Admin"))
                            .collect(Collectors.toList());
                    break;
                case "Admin":
                    filteredUsers = adminManager.filterUsers("users.txt", criteria, value).stream()
                            .filter(user -> user.getId().startsWith("A") && !user.getUserType().equalsIgnoreCase("Manager"))
                            .collect(Collectors.toList());
                    break;
                default:
    ...
    private void loadUsersIntoTable(String userType) {
        try {
            ...
            switch (userType) {
                case "Customer":
                    users = adminManager.loadUsersFromFile("customers.txt");
                    break;
                case "Scheduler":
                    users = adminManager.loadUsersFromFile("schedulers.txt");
                    break;
                case "Manager":
                    users = adminManager.loadUsersFromFile("users.txt").stream()
    ...
    private void editUser() {
        ...
        User user = adminManager.getUserByIdAcrossFiles(userId);
        ...
        if (user != null) {
            ...
            if (result == JOptionPane.OK_OPTION) {
                try {
                    ...
                    adminManager.editUserById(user);
    ...
    // In AdminDashboard.java (Similar for other dashboards)
    public interface LogoutCallback {
        void onLogout();
    }
    private LogoutCallback logoutCallback; // This will store the callback

    // Method to set the logout callback from the Main class
    ...
    private void logout() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            ...
            if (logoutCallback != null) { // Check if the logout callback is set
                logoutCallback.onLogout(); // Trigger the callback to handle logout
            }
        }
    }
